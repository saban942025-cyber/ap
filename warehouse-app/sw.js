...
const urlsToCache = [
  ...
  // Tailwind not used, Leaflet, Tone.js, Font Awesome are
  ...
  // App Icon Placeholder (update if specific icon exists)
  'https://www.google.com/search?q=https://placehold.co/192x192/007bff/ffffff%3Ftext%3D📋' // From manifest
];
...
self.addEventListener('install', event => {
  ...
  event.waitUntil(
    caches.open(CACHE_NAME)
        ...
        const cachePromises = urlsToCache.map(urlToCache => {
             return cache.add(urlToCache).catch(err => {
                 ...
                 if (!urlToCache.startsWith('./') && !urlToCache.startsWith('../')) {
                    console.warn(`[SW Warehouse v1] Failed to cache external asset ${urlToCache}: ${err}`);
                 } else {
                    console.error(`[SW Warehouse v1] Failed to cache critical asset ${urlToCache}: ${err}`);
                    // Optional: throw err; // Uncomment to make install fail if critical local assets fail
                 }
             });
         });
      ...
      .catch(err => {
           console.error('[SW Warehouse v1] Cache open/addAll failed during install:', err);
      })
  );
});
...
self.addEventListener('activate', event => {
  ...
  event.waitUntil(
      ...
      return Promise.all(
        cacheNames.map(cacheName => {
          if (!cacheWhitelist.includes(cacheName)) {
            console.log('[SW Warehouse v1] Deleting old cache:', cacheName);
    ...
    .catch(err => {
        console.error('[SW Warehouse v1] Activation failed:', err);
    })
  );
});
...
self.addEventListener('fetch', event => {
    ...
    if (event.request.method !== 'GET') {
        // console.log('[SW Warehouse v1] Ignoring non-GET request:', event.request.method, event.request.url);
        return; // Allow request to proceed normally
    }

    const url = new URL(event.request.url);

    // Ignore Firebase, Map Tiles, Nominatim Geocoding, OneSignal
    if (url.hostname.includes('firestore.googleapis.com') ||
        ...
        url.hostname.includes('onesignal.com') // Ignore OneSignal requests
       ) {
       // console.log('[SW Warehouse v1] Ignoring API/Map/Extension request:', url.href);
    ...
    event.respondWith(
        caches.match(event.request)
          .then(cachedResponse => {
            if (cachedResponse) {
              // console.log('[SW Warehouse v1] Serving from cache:', event.request.url);
              return cachedResponse;
            }

            // console.log('[SW Warehouse v1] Fetching from network:', event.request.url);
            ...
            return fetch(fetchRequest).then(
              networkResponse => {
                ...
                caches.open(CACHE_NAME)
                  .then(cache => {
                    // console.log('[SW Warehouse v1] Caching new resource:', event.request.url);
                ...
                return networkResponse;
                 ...
                 console.error('[SW Warehouse v1] Fetch failed; network error.', error, event.request.url);
